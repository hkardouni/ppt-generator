...
const Editor = () => {
  ...
  const GenerateSlides = async () => {
    ...
    for (let i = 0; i < outlineLength && i < 5; i++) {
      ...
      const prompt = SLIDER_PROMPT.replace(
        ...
        .replace(
          "{DESIGN_STYLE}",
          projectDetail.designStyles?.designGuide ?? ""
        )
        .replace(
          "{COLORS_CODE}",
          JSON.stringify(projectDetail.designStyles?.colors || {})
        );
  ...
  const exportAllframesTpPPT = async () => {
    // ensure container exists before accessing current
